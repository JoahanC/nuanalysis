...
for idx in tqdm(run_order):
    ...
    if f"1000_binning_flag.txt" in os.listdir(f"../bifrost_data/24/{seqid}/event_cl/"):
        if f"1000_flag.txt" not in os.listdir(f"../bifrost_data/24/{seqid}/event_cl/"):
            path = f"../bifrost_data/24/{seqid}/"
            evdir = f"{path}event_cl/"
            out_path = f"{path}products/"
            run_object = NuAnalysis(1000, 3, path=path, evdir=evdir, seqid=seqid, out_path=out_path, clean=True, bifrost=True, object_name=object_name, sessionid=22)
...
for idx in tqdm(run_order):
    object_name = run_order[idx][0]
    seqid = run_order[idx][1]
    if f"1000_binning_flag.txt" in os.listdir(f"../bifrost_data/23/{seqid}/event_cl/"):
        if f"1000_flag.txt" not in os.listdir(f"../bifrost_data/23/{seqid}/event_cl/"):
...
for idx in tqdm(run_order):
    object_name = run_order[idx][0]
    seqid = run_order[idx][1]
    if f"1000_binning_flag.txt" in os.listdir(f"../bifrost_data/31/{seqid}/event_cl/"):
        if f"1000_flag.txt" not in os.listdir(f"../bifrost_data/31/{seqid}/event_cl/"):
