...
for idx in run_order:
    ...
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/4/{seqid}/event_cl/"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/4/{seqid}/event_cl/"):
            if f"{dtime}_binning_flag.txt" not in os.listdir(f"../bifrost_data/4/{seqid}/event_cl/"):
                print(seqid)
                path = f"../bifrost_data/4/{seqid}/"
                evdir = f"{path}event_cl/"
                out_path = f"{path}products/"
                run_object = NuAnalysis(dtime, 3, path=path, evdir=evdir, seqid=seqid, out_path=out_path, clean=True, bifrost=True, object_name=object_name, sessionid=4)
