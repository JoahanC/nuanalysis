...
for idx in tqdm(run_order):
    ...
    if f"1000_binning_flag.txt" in os.listdir(f"../bifrost_data/2/{seqid}/event_cl/"):
        if f"1000_flag.txt" not in os.listdir(f"../bifrost_data/2/{seqid}/event_cl/"):
...
for idx in tqdm(run_order):
    object_name = run_order[idx][0]
    seqid = run_order[idx][1]
    if f"1000_binning_flag.txt" in os.listdir(f"../bifrost_data/12/{seqid}/event_cl/"):
        if f"1000_flag.txt" not in os.listdir(f"../bifrost_data/12/{seqid}/event_cl/"):
