...
for idx in run_order:
    ...
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/8/{seqid}/event_cl/"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/8/{seqid}/event_cl/"):
            if "1000_binning_flag.txt" not in os.listdir(f"../bifrost_data/8/{seqid}/event_cl/"):
                path = f"../bifrost_data/8/{seqid}/"
                evdir = f"{path}event_cl/"
                out_path = f"{path}products/"
                run_object = NuAnalysis(1000, 3, path=path, evdir=evdir, seqid=seqid, out_path=out_path, clean=True, bifrost=True, object_name=object_name, sessionid=8)
...
for idx in run_order:
    ...
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/18/{seqid}/event_cl/"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/18/{seqid}/event_cl/"):
            if "1000_binning_flag.txt" not in os.listdir(f"../bifrost_data/18/{seqid}/event_cl/"):
                path = f"../bifrost_data/18/{seqid}/"
                evdir = f"{path}event_cl/"
                out_path = f"{path}products/"
                run_object = NuAnalysis(1000, 3, path=path, evdir=evdir, seqid=seqid, out_path=out_path, clean=True, bifrost=True, object_name=object_name, sessionid=18)
                run_object.event_extraction()
...
for idx in run_order:
    ...
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/28/{seqid}/event_cl/"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/28/{seqid}/event_cl/"):
            if "1000_binning_flag.txt" not in os.listdir(f"../bifrost_data/28/{seqid}/event_cl/"):
                path = f"../bifrost_data/28/{seqid}/"
                evdir = f"{path}event_cl/"
                out_path = f"{path}products/"
                run_object = NuAnalysis(1000, 3, path=path, evdir=evdir, seqid=seqid, out_path=out_path, clean=True, bifrost=True, object_name=object_name, sessionid=28)
                run_object.event_extraction()