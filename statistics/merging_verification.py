...
for directory in data_dirs:
    ...
    for seqid in seqids:
        ...
        binning_files = glob.glob(bin_path_stub + "5000_merging_flag.txt")
        for file in binning_files:
            data_info[index][obsid].append(file.replace(bin_path_stub, ''))
        binning_files = glob.glob(bin_path_stub + "1000_merging_flag.txt")
        for file in binning_files:
            data_info[index][obsid].append(file.replace(bin_path_stub, ''))
        binning_files = glob.glob(bin_path_stub + "500_merging_flag.txt")
        for file in binning_files:
            data_info[index][obsid].append(file.replace(bin_path_stub, ''))
...
for directory in data_info:
    ...
    for seqid in data_info[directory]:
        if "both_present" in data_info[directory][seqid]:
            count_rel += 1
        if "5000_merging_flag.txt" in data_info[directory][seqid]:
            count_fivek += 1
        if "1000_merging_flag.txt" in data_info[directory][seqid]:
            count_onek += 1
        if "500_merging_flag.txt" in data_info[directory][seqid]:
            count_halfk += 1
...
plt.savefig(f"summary_merging_1_{ssd_index}.pdf", dpi=1000)
plt.close()
