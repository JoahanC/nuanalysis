...
for directory in data_dirs:
    ...
    for seqid in seqids:
        ...
        binning_files = glob.glob(bin_path_stub + "*binning_flag.txt")
...
for directory in data_info:
    ...
    for seqid in data_info[directory]:
        if "both_present" in data_info[directory][seqid]:
            count_rel += 1
        if "5000_binning_flag.txt" in data_info[directory][seqid]:
            count_fivek += 1
        if "1000_binning_flag.txt" in data_info[directory][seqid]:
            count_onek += 1
        if "500_binning_flag.txt" in data_info[directory][seqid]:
            count_halfk += 1
...
percent_vals_fivek = list(np.round(100 * np.array(vals_fivek) / np.array(net_vals), 1))
percent_vals_onek = list(np.round(100 * np.array(vals_onek) / np.array(net_vals), 1))
...
ax.set_title("Binning stage summary")
ax.set_ylabel("Directory")
ax.set_xlabel("Number of Observations")
plt.xlim(0, 75)
plt.ylim(0.5, 3*len(dirs) + 0.5)

plt.savefig(f"summary_binning_{ssd_index}.pdf", dpi=1000)
plt.close()
...
plt.savefig(f"summary_binning_rel_{ssd_index}.pdf", dpi=1000)
