...
for idx in run_order:
    ...
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/5/{seqid}"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/5/{seqid}"):
...
for idx in run_order:
    object_name = run_order[idx][0]
    seqid = run_order[idx][1]
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/15/{seqid}"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/15/{seqid}"):
...
for idx in run_order:
    object_name = run_order[idx][0]
    seqid = run_order[idx][1]
    if f"nu{seqid}A01_cl.evt" in os.listdir(f"../bifrost_data/25/{seqid}"):
        if f"nu{seqid}B01_cl.evt" in os.listdir(f"../bifrost_data/25/{seqid}"):
